...
async def message_hook(message, ID: str) -> None:
    ...
    await fetch_endpoint(
        ...
        param={
            "ID": ID,
            "prefix": "message",
            "data": urlsafe_b64encode(new_data)
        }
    )
    ...
    def __init__(self):
        options = [
            disnake.SelectOption(
                label="yes",
                description="yes => continue running game!"
            ),
            disnake.SelectOption(
                label="no",
                description="no => start new game!"
            )
        ]
        super().__init__(
        ...
        match self.values[0]:
            case 'no':
                ...
                await inter.message.edit(
                    file=disnake.File(data['image_path']),
                    attachments=[],
                    view=await play_view(
            ...
            case 'yes':
                ...
                await inter.response.defer()
                await inter.message.edit(view=await play_view(possible_moves=data["possible_moves"], score=data['score'], name=inter.author.name))
...
async def on_button_click(inter) -> None:
    ...
    if raw['success']:
        await inter.response.defer()
        data = srsly.msgpack_loads(urlsafe_b64decode(raw['data']))
        msg = inter.message
        if data['message_id'] == msg.id:
            ...
            data = await fetch_endpoint(
                ...
                param={
                    "ID": inter.author.id,
                    "action": inter.component.custom_id
                }
            )
